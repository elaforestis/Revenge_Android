...
public class ExpandableListMainActivity extends ExpandableListActivity
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        if(requestType.equals("liveNomos") || requestType.equals("generalNomos")){
            parentItems = new ArrayList<String>(Arrays.asList(nomoiNormalized));
            MyExpandableAdapter adapter = new MyExpandableAdapter(parentItems, null);

            adapter.setInflater((LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE), this);

            // Set the Adapter to expandableList
            expandableList.setAdapter(adapter);
            expandableList.setOnGroupClickListener(new ExpandableListView.OnGroupClickListener() { //prevents expanding
                @Override
                public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition, long id) {
                    nomosSelected=nomoiNormalized[groupPosition];
                    Intent i = new Intent(ExpandableListMainActivity.this,highscoresAna.class);
                    startActivity(i);
                    finish();
                    return true;
                }
            ...
            parentItems = new ArrayList<String>(Arrays.asList(nomoi));
            List<String> x;
            for(int i=0;i<nomoi.length;i++){
                x = new ArrayList<String>();
                for (int j =0;j<perioxes.length;j++){
                    try {
                        if (perioxes[i][j] != null) {
                            x.add(perioxes[i][j]);
                        }
    ...
    private class Checker extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            while (MyExpandableAdapter.justselected==false) {
                //do nothing
            }
            MyExpandableAdapter.justselected=false;
            if(ExpandableListMainActivity.requestType.equals("livePerioxi") || ExpandableListMainActivity.requestType.equals("generalPerioxi")){
                Intent i = new Intent(ExpandableListMainActivity.this,highscoresAna.class);
                startActivity(i);
                finish();
            }else {
